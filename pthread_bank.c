...
// Final Project: Pthread Implementation
...
// Initial data to be sent to the bank teller threads.
struct thread_data
{
	int teller_id;
	int total_transactions;
...
void init(int arg1, int arg2, int arg3) {
	...
	curr_transactions = 0;
	acc_mutex = calloc(num_accounts, sizeof(pthread_mutex_t));
  bank_accounts = calloc(num_accounts, sizeof(struct account));

  // Initialize mutexes.
  pthread_mutex_init(&transactions_mutex, NULL);
  pthread_mutex_init(&monitor_mutex, NULL);
  pthread_mutex_init(&send_mutex, NULL);

  //Initialize mutex and account variables
...
void deposit(int teller_id, int acc_id, double value)
{
  ...
  if (value > MONEY_LAUNDERING_AMOUNT) {
    ...
    pthread_mutex_unlock(&monitor_mutex);
  	...
  	// Locks the account mutex, deposits into the account and the unlocks the account mutex.
    pthread_mutex_lock(&acc_mutex[acc_id]);
    bank_accounts[acc_id].balance += value;

    // Uncomment line below to print out deposit transactions.
    //printf("Deposited $%.2f into %d.\n", value, acc_id);
    pthread_mutex_unlock(&acc_mutex[acc_id]);
  }
}
...
void withdraw(int teller_id, int acc_id, double value)
{
  ...
  if (value > MONEY_LAUNDERING_AMOUNT) {
    ...
    pthread_mutex_unlock(&monitor_mutex);
  	...
  	// Locks the account mutex, withdraws from the account and then unlocks the account mutex.
    pthread_mutex_lock(&acc_mutex[acc_id]);
    bank_accounts[acc_id].balance -= value;
...
{
	// Checks if there is money laundering.
  if (value > MONEY_LAUNDERING_AMOUNT) {
    ...
    while (transferred == 0) {
      ...
      pthread_mutex_lock(&acc_mutex[to_id]);

      // Tries to lock the account the money is being sent from.
      if (pthread_mutex_trylock(&acc_mutex[from_id]) == 0) {
        ...
        pthread_mutex_unlock(&acc_mutex[to_id]);
        pthread_mutex_unlock(&acc_mutex[from_id]);
        ...
        timeout = 0;
      	...
      	// Unlocks the first mutex and increases timeout counter.
        pthread_mutex_unlock(&acc_mutex[to_id]);
        timeout++;

        // If the timeout counter hits 1000, it times out to prevent deadlocks.
...
// Bank teller threads.
void *bank_teller(void *t)
{
	// Get the bank teller thread data.
	struct thread_data *data;
  data = (struct thread_data*) t;

  // Each bank teller thread will continue until the total number of transactions is reached.
  while (curr_transactions < data->total_transactions) {
    ...
    // Execute the random transaction.
   	if (rand_tran == 1) {
     	deposit(data->teller_id, rand()%num_accounts, RandomFloat(0.0, 10500.0));
    } else if (rand_tran == 2) {
      withdraw(data->teller_id, rand()%num_accounts, RandomFloat(0.0, 10500.0));
    } else if (rand_tran == 3) {
      transfer(data->teller_id, rand()%num_accounts, rand()%num_accounts, RandomFloat(0.0, 10500.0));
    } else {
      printf("Invalid transaction selected.\n");
    }
  }

  // Exit the bank teller thread.
  pthread_exit(NULL);
}
...
int main(int argc, char *argv[])
{
	...
	if (argc == 4) {
  	...
  	struct thread_data thread_data_array[num_tellers];
  	...
  	error = pthread_create(&threads[0], &attr, monitor, (void *) &monitor_data_values);
    if (error) {
     	printf("ERROR: create() monitor: %d\n", error);
      exit(-1);
    }
  	
  	// Creates the bank teller threads.
  	for (i=1; i<num_tellers+1; i++) {
  		thread_data_array[i].teller_id = i;
    	thread_data_array[i].total_transactions = num_transactions;
    	error = pthread_create(&threads[i], &attr, bank_teller, (void *) &thread_data_array[i]);
      if (error) {
      	printf("ERROR: create() bank_teller: %d\n", error);
      	exit(-1);
    	}
  	}
